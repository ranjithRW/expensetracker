...
import chartImage from '../../assets/images/cha.webp'
    ...
    return (
        <div className='flex'>
            ...
            <div className='hidden md:block w-[40vw] h-screen bg-violet-50 bg-auth-bg-img bg-cover bg-no-repeat bg-center overflow-hidden p-8 relative'>
                ...
                <img src={chartImage} className='w-64 lg:w-[90%] absolute bottom-10 shadow-lg shadow-blue-400/15' alt='chart' />'
            </div>
        </div>
    )
}
...
const StatsInfoCard = ({ icon, label, value, color }) => {
    return <div className='flex gap-6 bg-white p-4 rounded-xl shadow-md shadow-purple-400/10 border border-gray-200/50 z-10'>
        <div className={`w-12 h-12 flex items-center justify-center text-[26px] text-white rounded-[50%] ${color}`}>{icon}</div>
        <div>
            <h6 className='text-xs text-gray-500 mb-1'>{label}</h6>
            <span className='text-[20px] font-semibold text-black'>${value}</span>
        </div>
    </div>
}